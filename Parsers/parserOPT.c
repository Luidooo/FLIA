...
//	system("/tmp/dir/software/planners/madagascar/M -Q domain.pddl problem.pddl | grep -i click > out");
	...
	system("/tmp/dir/software/planners/downward-fdss23/fast-downward.py --alias seq-opt-fdss-2023 --overall-time-limit 179 domain.pddl problem.pddl | grep -i click > out");
	system("sed 's/click loc\\([0-9]*\\)-\\([0-9]*\\) (\\([0-9]\\))/\\(click \\1 \\2\\);/' out > outFile");
