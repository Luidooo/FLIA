...
//	system("/tmp/dir/software/planners/madagascar/M -Q domain.pddl problem.pddl | grep -i click > out");
	...
	//system("/tmp/dir/software/planners/downward/fast-downward.py --alias lama domain.pddl problem.pddl | grep -i click > out");
	system("/tmp/dir/software/planners/downward/fast-downward.py --alias seq-sat-fdss-1 --overall-time-limit 179 domain.pddl problem.pddl | grep -i click > outFile");
	
	int max=0, num=1;
	while(1){
	char filename[100]; 
        sprintf(filename, "sas_plan.%d", num);
  ...
  if(max>0){
    char maxPlan[100]; sprintf(maxPlan, "sas_plan.%d", max);
    char command[400]; sprintf(command, "sed -E 's/\\(click loc([0-9]+)-([0-9]+)\\)/\\(click \\1 \\2\\);/' %s | sed '/cost = [0-9]* (unit cost)/d' > outFile", maxPlan);
    system(command);
    system("sed '$ s/.$//' outFile > a");
    system("cat a");
    return 0;
  }else{
    exit(120);
